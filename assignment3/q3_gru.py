...
class SequencePredictor(Model):
    ...
    def add_training_op(self, loss):
        ...
        var = [output[1] for output in grad_var]
        grads = [output[0] for output in grad_var]
        if self.config.clip_gradients:
            tmp_gradients = tf.clip_by_global_norm(grads, clip_norm=self.config.max_grad_norm)[0]
            grads = tmp_gradients

        grad_var = [(grads[i], var[i]) for i in range(len(grads))]
