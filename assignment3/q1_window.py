...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for i in range(window_size, len(labels)-window_size):
            feature_list = new_sentence[i-window_size:i+window_size+1]
            feature = []
            for elem in feature_list:
                feature += elem
            windowed_data.append((feature, labels[window_size - 1]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embedding_tensor = tf.nn.embedding_lookup(self.pretrained_embeddings, self.input_placeholder)
        embeddings = tf.reshape(embedding_tensor, (-1, Config.n_window_features * Config.embed_size))
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable('Weight', shape=(Config.embed_size, Config.hidden_size), initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable('b1', shape=(Config.hidden_size, 1), initializer=tf.zeros_initializer())
        ...
        b2 = tf.get_variable('b2', shape=(Config.n_classes, 1), initializer=tf.zeros_initializer())
