...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        new_label = [zero_label] * max_length
        new_label[:len(labels)] = labels
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embedding_tensor, (-1, self.max_length, Config.n_features * Config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                state_dropout = tf.nn.dropout( state, (1 - dropout_rate) )
                output = tf.add(tf.matmul(state_dropout, U), b2)
                preds.append(output)
                ### END YOUR CODE
